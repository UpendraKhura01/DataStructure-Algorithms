package arrays.A_19Words_within_two_edits_of_dictionary_2452;
...
public class Solution {
    public static void main(String[] args) {
        ...
        List<String> list = twoEditWords(queries, dictionary);
    }
    ...
    static boolean ispossible(String word, String[] dictionary){
        for(int i = 0, n = dictionary.length; i < n; i++){
            String dic = dictionary[i];
            if(canconvert(word, dic)){
                return true;
            }
        }
        return false;
    ...
    static boolean canconvert(String word, String dic){
        if(word.length() != dic.length()) return false;
        int count = 0;
