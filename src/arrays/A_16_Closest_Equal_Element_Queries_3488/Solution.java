package arrays.A_16_Closest_Equal_Element_Queries_3488;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.List;

/*
3488. Closest Equal Element Queries
Medium

You are given a circular array nums and an array queries.
For each query i, you have to find the following:
The minimum distance between the element at index queries[i] and any other index j in the circular array,
where nums[j] == nums[queries[i]]. If no such index exists, the answer for that query should be -1.
Return an array answer of the same size as queries, where answer[i] represents the result for query i.

Example 1:

Input: nums = [1,3,1,4,1,3,2], queries = [0,3,5]

Output: [2,-1,3]

Explanation:

Query 0: The element at queries[0] = 0 is nums[0] = 1. The nearest index with the same value is 2,
 and the distance between them is 2.
Query 1: The element at queries[1] = 3 is nums[3] = 4. No other index contains 4, so the result is -1.
Query 2: The element at queries[2] = 5 is nums[5] = 3. The nearest index with the same value is 1,
 and the distance between them is 3 (following the circular path: 5 -> 6 -> 0 -> 1).
Example 2:

Input: nums = [1,2,3,4], queries = [0,1,2,3]

Output: [-1,-1,-1,-1]

Explanation:

Each value in nums is unique, so no index shares the same value as the queried element. This results in -1 for all queries.
 */

public class Solution {
    public static void main(String[] args) {
        int[] nums = {1,3,1,4,1,3,2};
        int[] queries = {0, 3, 5};
        List<Integer> ans = solveQueries(nums, queries);
        for (int i = 0, size = ans.size(); i < size; i++) {
            System.out.println(ans.get(i));
        }

    }
    static List<Integer> solveQueries(int[] nums, int[] queries) {
        int n = nums.length;
        int m = queries.length;

        HashMap<Integer, ArrayList<Integer>> mp = new HashMap<>();
        for (int i = 0; i < n; i++) {
            mp.computeIfAbsent(nums[i], k -> new ArrayList()).add(i);
        }
        List<Integer> ans = new ArrayList<>();

        for (int i = 0; i < m; i++) {
            int num = nums[queries[i]];
            int startIndex = queries[i];
            int minDist = Integer.MAX_VALUE;

            ArrayList<Integer> list = mp.get(num);
            int size = list.size();
            if (size == 1) {
                minDist = -1;
            } else {
                int indexofElement = binarysearch(list, startIndex);
                int left = (indexofElement == 0) ? size - 1 : indexofElement - 1;
                int right = (indexofElement == size - 1) ? 0 : indexofElement + 1;

                int straight = Math.abs(startIndex - list.get(left));
                int circular = Math.abs(n - straight);
                int leftDist = Math.min(straight, circular);

                straight = Math.abs(startIndex - list.get(right));
                circular = Math.abs(n - straight);
                int rightDist = Math.min(straight, circular);

                minDist = Math.min(leftDist, rightDist);

            }

            ans.add(minDist);
        }
        return ans;
    }

    static int binarysearch(ArrayList<Integer> list, int target) {
        int left = 0;
        int right = list.size() - 1;
        while (left <= right) {
            int mid = left + (right - left) / 2;
            int num = list.get(mid);
            if (num == target)
                return mid;
            else if (num < target)
                left = mid + 1;
            else
                right = mid - 1;
        }
        return -1;
    }
}
