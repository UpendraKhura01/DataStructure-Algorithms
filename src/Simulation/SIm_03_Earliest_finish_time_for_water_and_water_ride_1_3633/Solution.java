package Simulation.SIm_03_Earliest_finish_time_for_water_and_water_ride_1_3633;
    ...
    static int earliestFinishTime(int[] landStartTime, int[] landDuration, int[] waterStartTime, int[] waterDuration) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < m; j++) {
                ...
                int finish1 = actualWaterStart + waterDur;

                // Water -> Land
                int actualLandStart = Math.max(waterEnd, landStart);
                int finish2 = actualLandStart + landDur;
