package Range_Query.RQ_02_Block_Placement_Queries_3161;
...
public class Solution {
    ...
    class answer {
        ...
        public List<Boolean> getResults(int[][] queries) {
            ...
            for(int[] q : queries){
                if(q[0] == 1){
                    ...
                    int idx = q[1];
                    int size = q[2];
                    int max_left = query(0, idx, size, 0, 0, N - 1);
        ...
        private int query(int q_start, int q_end , int size, int cur, int lr, int rr){
            ...
            int mid = lr + (rr - lr) / 2;
            int left = query(q_start, q_end, size, 2 * cur + 1, lr, mid);
            int right = query(q_start, q_end, size, 2 * cur + 2, mid + 1, rr);
