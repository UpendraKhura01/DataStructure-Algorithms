package Range_Query.RQ_04_Concatenate_Non_Zero_Digits_And_Multiply_by_Sum_2_3756;
...
public class Solution {
    ...
    public int[] sumAndMultiply(String s, int[][] queries) {
        ...
        filldata(digits, numbers, presum);
        int idx = 0;
        for (int[] q : queries) {
            int l = q[0];
            int r = q[1] + 1;
            int sum = presum[r] - presum[l];
            int numOfDigit = digits[r] - digits[l];
            long prefix = numbers[l];

            long remove = (prefix * pow[numOfDigit]) % M;

            long num = (numbers[r] - remove + M) % M;
            ans[idx++] = (int) ((num * sum) % M);
        }
        return ans;

    }

    private void filldata(int[] digits, int[] numbers, int[] presum) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (d == 0) {
                ...
                numbers[i + 1] = (int) ((1L * numbers[i] * 10 + d) % M);
            }
        }
    }
